...
from datetime import date  # to create excel sheets with the current date as the file name
...
def start():
    for dictionary_element in appliances_dictionary:
        category = dictionary_element["category"]
        link = dictionary_element["link"]
        brand = dictionary_element["brand"]
        type = dictionary_element["type"]
        number_of_ads = 2000

        thread = threading.Thread(target=scrape, args=(category, link, brand, type, number_of_ads))
        thread.start()
...
def scrape(category_title, category_link, brand_array, type_array, number_of_ads_to_scrap):
    # variables to keep track of number of ads scraped vs to scrape
    ...
    # this variable decides which page to start scraping from
    page_number = 1
    ...
    while True:
        ...
        # Ending the script if we have successfully scraped the amout of ads we wanted
        ...
        for ad in all_ads_on_site:
            ...
            if sponsored_ad is not None:
                ...
                if ad_link in sponsored_ad_links_array:
                    print(f"Duplicate sponsored ad, from under-category {category_title} with link: {ad_link}")
                    continue
                else:
                    sponsored_ad_links_array.append(ad_link)
            ...
            if ad_payment_type.lower() == "til salgs":
                ...
                # Otherwise, splitting the price "kr" and adding it to the sheet
                    ...
                    ws.append([ad_title, category_title, product_type, price, product_brand, ad_postnr])
